...
import { SpeakOptions } from "@mentra/sdk";
...
class PokerCoachMentraApp extends AppServer {
    ...
    session.events.onButtonPress(async (button) => {
      ...
      if (state === "awaiting_photo") {
        try {
          ...
          const photoUrl = `${this.getServerUrl()}/api/photo/${
            photo.requestId
          }`;
  ...
  private async fetchHandAnalysis(
    ...
    const rawPredictions = Array.isArray(rfJson.predictions)
      ? rfJson.predictions
      : [];
    const classes = [
      ...new Set(
        rawPredictions.map((p) => p.class).filter((c) => typeof c === "string")
      ),
    ];
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an intelligent poker assistant for newbie players. You have an image of my hand, pre-flop.",
        },
        {
          role: "user",
          content: `My hand is ${classes.join(
            " and "
          )}. Return to me a JSON file in the format: {win_probability: 0-100,tip: 
    ...
    this.logger.info(
      `[OpenAI] Request payload: ${JSON.stringify(openaiPayload, null, 2)}`
    );
